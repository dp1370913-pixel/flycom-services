...
<div class="settings-tab-pane" id="tab-ia-web">
    <div class="card border-0 shadow-sm p-4 bg-white rounded-4 mb-4">
        ...
        <form action="{{ route('admin.settings.updateWebIA') }}" method="POST" enctype="multipart/form-data" class="row g-3 fs-8 text-start">
            ...
            <div class="col-12 mt-4">
                <label for="ai_provider" class="form-label fw-bold text-navy text-uppercase">Fournisseur de Service IA *</label>
                <select name="ai_provider" id="ai_provider" class="form-select bg-light border-light py-2 fs-8" required style="box-shadow:none !important;">
                    <option value="chatgpt_openrouter" {{ ($configs['ai_provider'] ?? '') === 'chatgpt_openrouter' ? 'selected' : '' }}>ChatGPT (Haute Performance)</option>
                    <option value="gemini" {{ ($configs['ai_provider'] ?? 'gemini') === 'gemini' ? 'selected' : '' }}>Google Gemini (Standard - Gratuit)</option>
                </select>
            </div>

            <div class="col-12 mt-4">
                <label for="ai_model" class="form-label fw-bold text-navy text-uppercase">Modèle d'Intelligence Artificielle *</label>
                <select name="ai_model" id="ai_model" class="form-select bg-light border-light py-2 fs-8" required style="box-shadow:none !important;"></select>
            </div>

            <div class="col-12 mt-4">
                <label for="chatbot_system_prompt" class="form-label fw-bold text-navy text-uppercase">Consignes de comportement (Prompt)</label>
                <textarea name="chatbot_system_prompt" id="chatbot_system_prompt" class="form-control bg-light border-light py-2 fs-8 text-muted" rows="5" required style="box-shadow:none !important;">{{ $configs['chatbot_system_prompt'] ?? "Tu es l'assistant virtuel de Flycom Services à Brazzaville. Reste courtois, professionnel et très concis (2-3 phrases maximum)." }}</textarea>
...
- Le gérant est M. Budry Nakouzebi, joignable directement au +242 06 628 57 41." style="box-shadow:none !important;">{{ $configs['chatbot_knowledge_base'] ?? '' }}</textarea>
                <small class="text-muted fs-10 mt-1 d-block">Ajoutez ici toutes les informations spécifiques à votre entreprise.</small>
            </div>
            ...
            <div class="col-12 mt-4">
                ...
                <small class="text-muted fs-10 mt-1 d-block">Téléversez un fichier PDF pour alimenter automatiquement la base de connaissances.</small>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. Moteur de routage des modèles d'IA ChatGPT et Gemini Direct uniquement
    const providerSelect = document.getElementById('ai_provider');
    const modelSelect = document.getElementById('ai_model');
    const savedModel = "{{ $configs['ai_model'] ?? '' }}";

    const modelsMap = {
        chatgpt_openrouter: [
            { value: 'openai/gpt-4o-mini', text: 'ChatGPT: GPT-4o Mini (Très Rapide & Économique)' },
            { value: 'openai/gpt-4o', text: 'ChatGPT: GPT-4o (Modèle intelligent flagship)' }
        ],
        gemini: [
            { value: 'gemini-2.5-flash', text: 'Google: Gemini 2.5 Flash (Rapide & Recommandé)' },
            { value: 'gemini-1.5-flash', text: 'Google: Gemini 1.5 Flash' },
            { value: 'gemini-2.5-pro', text: 'Google: Gemini 2.5 Pro (Haute précision)' }
        ]
    };
    ...
    function populateModels() {
        ...
        const options = modelsMap[provider] || [];
